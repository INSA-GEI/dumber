...
#define PRIORITY_TSTARTROBOTWITHOUTWD 20
#define PRIORITY_TSTARTROBOTWITHWD 20
#define PRIORITY_TCAMERA 21
#define PRIORITY_RELOAD_WATCHDOG 40
#define PRIORITY_STOP_ROBOT 40
...
void Tasks::Init() {
    ...
    if (err = rt_mutex_create(&mutex_reloadWatchdog, NULL)) {
        cerr << "Error mutex create: " << strerror(-err) << endl << flush;
        exit(EXIT_FAILURE);
    }
    ...
    if (err = rt_sem_create(&sem_startRobotWithoutWD, NULL, 0, S_FIFO)) {
        cerr << "Error semaphore create: " << strerror(-err) << endl << flush;
        exit(EXIT_FAILURE);
    }
    ...
    if (err = rt_sem_create(&sem_monitorDown, NULL, 0, S_FIFO)) {
        cerr << "Error semaphore create: " << strerror(-err) << endl << flush;
        exit(EXIT_FAILURE);
    }
    ...
    if (err = rt_task_create(&th_startRobotWithoutWD, "th_startRobotWithoutWD", 0, PRIORITY_TSTARTROBOTWITHOUTWD, 0)) {
        cerr << "Error task create: " << strerror(-err) << endl << flush;
        exit(EXIT_FAILURE);
    }
    ...
    if (err = rt_task_create(&th_reload_watchdog, "th_reload_watchdog", 0, PRIORITY_RELOAD_WATCHDOG, 0)) {
        cerr << "Error task create: " << strerror(-err) << endl << flush;
        exit(EXIT_FAILURE);
    }
    if (err = rt_task_create(&th_stopRobot, "th_stopRobot", 0, PRIORITY_STOP_ROBOT, 0)) {
        cerr << "Error task create: " << strerror(-err) << endl << flush;
        exit(EXIT_FAILURE);
    }
...
void Tasks::Run() {
    ...
    if (err = rt_task_start(&th_startRobotWithoutWD, (void(*)(void*)) & Tasks::StartRobotTaskWithoutWD, this)) {
        cerr << "Error task start: " << strerror(-err) << endl << flush;
        exit(EXIT_FAILURE);
    }
    ...
    if (err = rt_task_start(&th_reload_watchdog, (void(*)(void*)) & Tasks::ReloadWatchdog, this)) {
        cerr << "Error task start: " << strerror(-err) << endl << flush;
        exit(EXIT_FAILURE);
    }
...
void Tasks::ServerTask(void *arg) {
    ...
    while (1) {
        ...
        rt_sem_p(&sem_monitorDown, TM_INFINITE);
    }
}
    ...
    while (1) {
        ...
        while (execute) {
            ...
            if (msgRcv->CompareID(MESSAGE_MONITOR_LOST)) {
                ...
                rt_sem_v(&sem_monitorDown);
                execute = false;
            } else if (msgRcv->CompareID(MESSAGE_ROBOT_COM_OPEN)) {
                rt_sem_v(&sem_openComRobot);
            } else if (msgRcv->CompareID(MESSAGE_ROBOT_START_WITHOUT_WD)) {
                rt_sem_v(&sem_startRobotWithoutWD);
 ...
 */
void Tasks::StartRobotTaskWithoutWD(void *arg) {
    cout << "Start " << __PRETTY_FUNCTION__ << endl << flush;
    // Synchronization barrier (waiting that all tasks are starting)
    rt_sem_p(&sem_barrier, TM_INFINITE);
    ...
    while (1) {
        ...
        Message * msgSend;
        rt_sem_p(&sem_startRobotWithoutWD, TM_INFINITE);
...
void Tasks::StartRobotTaskWithWD(void *arg) {
    ...
    while (1) {
        ...
        if (msgSend->GetID() == MESSAGE_ANSWER_ACK) {
            ...
            rt_mutex_acquire(&mutex_reloadWatchdog, TM_INFINITE);
            reloadWatchdog = 1;
            rt_mutex_release(&mutex_reloadWatchdog);
        }
    }
}
...
void Tasks::UpdateErrorCounter(Message * msg) {
    ...
    if (msg->CompareID(MESSAGE_ANSWER_NACK) ||msg->CompareID(MESSAGE_ANSWER_COM_ERROR) ||msg->CompareID(MESSAGE_ANSWER_ROBOT_UNKNOWN_COMMAND) || msg->CompareID(MESSAGE_ANSWER_ROBOT_TIMEOUT)|| msg->CompareID(MESSAGE_ANSWER_COM_ERROR)) {
        ...
        cout << "updateError counter: " << counter << endl << flush;
        if (counter >= 3) {
            rt_sem_v(&sem_stopRobot);
        }
    } else {
        rt_mutex_acquire(&mutex_comRobotErrorCounter, TM_INFINITE);
        comRobotErrorCounter = 0;
        rt_mutex_release(&mutex_comRobotErrorCounter);
    }
}
